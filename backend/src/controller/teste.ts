...
async function adicionarColunasFaltantes(tabela: string, colunas: { position: number, name: string }[], pool: sql.ConnectionPool, tabelasEColunasCriadas: string[]): Promise<void> {
  ...
  if (colunasNaoExistem.length > 0) {
    ...
    console.log(`Adicionando colunas ${colunasNaoExistem.map(coluna => coluna.name).join(', ')} na tabela ${tabela} com a query:`, adicionarColunasQuery);
...
async function registrarErroGenerico(
  ...
  console.log('Colunas configuradas:', colunas);
  console.log('Tabela de Atualização:', tabelaAtualizacao);
  console.log('Tabela Errada Formatada:', tabelaErradaFormatted);
